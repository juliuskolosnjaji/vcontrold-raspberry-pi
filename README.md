...
- `heizung/cmd/<Variable>`: sowohl Home Assistant als auch `orchestrator.py` selbst (bei
  einer Set-Weiterleitung, siehe unten) publishen hierher. Da MQTT selbst nicht verrät, welcher
  ...
  `Set-Weiterleitung`) -- die eigentliche Set-Verifikation läuft in allen Fällen identisch ab.

**Set-Weiterleitung** (`config/mqtt_mapping.json`, Abschnitt "Set-Weiterleitung" auf der MQTT-
Variablen-Seite): leitet den Wert einer beliebigen MQTT-Variable (egal ob aus vito.xml oder CAN)
zusätzlich als Set-Anfrage an eine **schreibbare Vcontrold-Variable** weiter -- z.B. um einen von
der UVR berechneten Sollwert tatsächlich in der Vitotronic zu setzen. Das Ziel-Dropdown zeigt
bewusst nur Vcontrold-Variablen, die als "Schreibbar" aktiviert sind: für die Gegenrichtung
(Vcontrold-/MQTT-Wert → CAN) ist bereits die 16-Slot-Tabelle "TA-Netzwerkausgänge senden"
(Abschnitt 3.3) der vollständige, alleinige Weg -- ein zusätzliches Mapping dorthin würde nie
tatsächlich am CAN-Bus ankommen, da `can_node.py` nur sendet, was in dieser Tabelle steht.
Ersetzt die frühere, an die CAN-Einstellungen-Seite gekoppelte "Weiterleitung" (`forward_as_set`).
...
2. **On-demand Set-Befehle**, sowohl von Home Assistant als auch (per Set-Weiterleitung, siehe "MQTT-Architektur" oben) von einer anderen MQTT-Variable -- beide über denselben Topic `heizung/cmd/<Variable>`, mit Quellenkennung im JSON-Payload.
...
**Sicherheitshinweis:** Die UI kann Sollwerte an die echte Heizung senden. Nur im vertrauenswürdigen LAN betreiben (nicht ins Internet weiterleiten), starkes Passwort in `ui.env` setzen. Der Login läuft über eine Session-Cookie; die Anmeldung selbst ist über HTTP unverschlüsselt — bei Bedarf zusätzlich per Reverse-Proxy (z.B. Caddy/nginx) mit HTTPS absichern.
